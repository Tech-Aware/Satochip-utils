...
class View(customtkinter.CTk):
        ...
        try:
            ...
            if self.card_present:
                if not self.setup_done:
                    ...
                    if not self.is_seeded and self.card_type != "Satodime":
            ...
            if self.controller.cc.card_type != "Satodime" and self.setup_done:
    ...
    def get_passphrase(self, msg):
        try:
            ...
            try:
                ...
                logger.info(f"Returning results: is_PIN={is_PIN}, pin={'***' if pin else None}")
                return pin
    ...
    def confirm_seed(self, seed, mnemonic, passphrase):
        try:
            logger.info("CC UTILS: IN View.confirm_seed | Initializing seed confirmation popup")

            # Création et configuration de la fenêtre popup
            popup = customtkinter.CTkToplevel(self)
            popup.protocol("WM_DELETE_WINDOW", lambda: popup.destroy())
            logger.debug("Popup window created and close protocol set")
            ...
            # Ajout d'une textbox dans le popup
            text_box = customtkinter.CTkTextbox(popup, corner_radius=5,
                                                bg_color=BUTTON_COLOR, fg_color="white",
                                                border_color=BUTTON_COLOR, border_width=1,
            ...
            # Définition de la fonction de vérification du seed
            def verify_seed():
                try:
                    logger.info("Verifying seed")
                    if text_box.get("1.0", "end-1c") != mnemonic + ' ' + passphrase:
                        logger.warning("Seeds don't match")
                        self.show('ERROR', "Seeds don't match")
                    else:
                        logger.info("Seed verified successfully")
                        self.controller.card_setup_native_seed(seed)
                        text_box.delete("1.0", "end-1c")
                        text_box.destroy()
                        popup.destroy()
                except Exception as e:
                    logger.error(f"An error occurred in verify_seed: {e}", exc_info=True)

            # Ajout d'un bouton de soumission dans le popup
            submit_button = customtkinter.CTkButton(popup, bg_color=BUTTON_COLOR, fg_color=MAIN_MENU_COLOR,
                                                    corner_radius=0, hover_color=HOVER_COLOR, text="Submit",
                                                    command=verify_seed)
            ...
            logger.error(f"An unexpected error occurred in confirm_seed: {e}", exc_info=True)
    ...
    def setup_my_card_pin(self):
        ...
        try:
            ...
            try:
                logger.debug("Creating main menu")
                if not self.controller.setup_done:
            ...
            try:
                ...
                if self.controller.card_type == "Satodime":
        ...
        try:
            ...
            def update_radio_selection():
                ...
                if radio_value.get() == "generate":
                    ...
                    label.place(relx=0.38, rely=0.48, anchor="w")
                    generate_seed_button.place(relx=0.81, rely=0.59, anchor="w")
                    radio_button_passphrase.place(relx=0.38, rely=0.65, anchor="w")
                    ...
                    if radio_value_passphrase.get() == "passphrase":
                        logger.debug("Generate seed with passphrase")
                        generate_with_passphrase = True
                        passphrase_entry.place(relx=0.37, rely=0.72, anchor="w")
                        passphrase_entry.insert(string="   Type your passphrase here", index=0)
            ...
            try:
                logger.debug("Placing finish button")
                self.cancel_button.place(relx=0.85, rely=0.9, anchor="w")
                #self.finish_button.place(relx=0.85, rely=0.9, anchor="w")
                logger.debug("Finish button placed")
    ...
    def edit_label(self):
        try:
            ...
            try:
                logger.debug("setup paragraph")
                text = View.create_label(self, f"Edit the label of your {self.card_type}.")
                text.place(relx=0.33, rely=0.17, anchor="w")
    ...
    def check_authenticity(self):
        if self.controller.card_present:
